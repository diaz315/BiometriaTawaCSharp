...
namespace Suprema
{
    public class Huella : Form
    {
        ...
        public float CompararHuellas(List<Empleado> empleados)
        {
            float similarity = 0;
            ...
            {
                #region
                //var t = Utilidad<Empleado>.convertStringToByte(huellaBase64);
                /*var bn = Utilidad<Empleado>.ByteToBitmap(t);
                bn.Save("Test.png", ImageFormat.Png);
                var te = Utilidad<Empleado>.ConverFisicImageToBase64("Test.png");
                MessageBox.Show(te.Length.ToString());
                var gh = Utilidad<Empleado>.ReduceBytes(t);
                MessageBox.Show(Utilidad<Empleado>.convertByteToString(gh).Length.ToString());
                //var asd = Utilidad<Empleado>.ByteToBitmap(gh);
                //asd.Save("TestOtro.png", ImageFormat.Png);
                var ttt = Utilidad<Empleado>.GetByteArrayFromFisicImage("Test.png");*/
                #endregion
                var huellaOriginal = Utilidad<Empleado>.ExtraerTemplate(huellaBase64);

                Person huellasCandidatas = new Person();
                ...
                foreach (Empleado empleado in empleados) 
                {
                    ...
                    huellasCandidatas.Fingerprints.Add(fingers);
                }
                ...
                if (empleados.Count() > 0)
                {
                    ...
                    Person candidate = huellasCandidatas;
                    afis.Threshold = 0;
                    float score = afis.Verify(probe, candidate);
                    similarity = FixScore(score);
                }
                else {
        ...
        private void MarcarHuella()
        {
            ...
            DrawCapturedImage(m_Scanner);
            Cursor.Current = Cursors.WaitCursor;
            //var HuellaScanner= Utilidad<Empleado>.imgToByteConverter(pbImageFrame.Image);
            var listaHuellaBd = ObtenerEmpleadoHuella();
            var coincidencia= CompararHuellas(listaHuellaBd);

            if (coincidencia>=0.50) {
